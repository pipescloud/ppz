...
Attach interactively to the pty bound to handle H: follow H.stdout (like 'watch') AND forward your keystrokes to H.stdin, after acquiring the advisory write-lease. If another controller already holds the lease, control degrades to a read-only attach (streams output; keystrokes not forwarded). Keystrokes — including Ctrl-C — pass through to the remote, so Ctrl-C interrupts the remote foreground process; press Ctrl-D to detach (this releases the lease). Known limitation: a plain remote shell does not echo your keystrokes back (headless shares suppress pty echo so injected input isn't doubled); full-screen/TUI programs render their own input, so typing is visible there. See 'ppz help' and docs/WIRE.md §12 for the lease model.`,
