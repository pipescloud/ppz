...
func Message(c Code) string {
	...
	case ENoCurrentSource:
		return "no current source for this shell session; run 'ppz source create <handle>' (or 'ppz set handle <handle>' to point at an existing one); if you're driving ppz from agent subprocesses with no shared tty, export PPZ_SESSION=<id> consistently across calls so they share session state"
	case EPayloadTooLarge:
